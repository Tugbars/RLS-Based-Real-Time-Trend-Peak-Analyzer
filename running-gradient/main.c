...
int main() {
	...
	*/
    ...
    init_gradient_analysis_params(
        ...
        0.35   // min_gradient_total: Minimum total gradient to flag a significant trend.
    );
    ...
    init_buffer_manager(buffer, BUFFER_SIZE, WINDOW_SIZE, 50, 11300.0, 1.0);

    // Perform sliding window analysis with the provided phaseAngles data
    perform_sliding_window_analysis(phaseAngles, size);

	return 0;
}

// isValid check, where dooes it come from? track_gradient_trends_with_cubic_regression
// detect_significant_gradient_trends, determine_trend_direction, have 2 checks. 
...
//this is getting called and if thisis valid we enter to the new checks. we'll come to that point. let's check what makes this valid.

//INCREASE COUNT MAKES IT VALID. AFTER THE INCREASE COUNT COMES THE THRESHOLD CHECK. 

//if valid wwe enter here:
